import {ArrContext, ObjContext, PairContext, TerminalNodeImpl, ValueContext,} from "./context";
import {JSONLexer, Token} from "./lexer";
...
class Parser {
  ...
  match(x: number) {
    if (this.LA(1) === x) {
      ...
      throw new Error(
        `Unexpected token ${this.LN(1)} in JSON at position ${this.LT(1).start}`
      );
    }
  }
...
export class JSONParser extends Parser {
  ...
  pair() {
    const localctx = new PairContext(this._ctx);
    this.enterRule(localctx);
    this.match(10);
    this.match(2);
    this.value();
  ...
  value() {
    ...
    switch (this.LA(1)) {
      ...
      default:
        throw new Error(
          `Unexpected token ${this.LN(1)} in JSON at position ${
            this.LT(1).start
          }`
        );
    }
